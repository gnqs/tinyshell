...
void setup(bool UseDefaultSet) {
	...
	else
	{
		...
		const char root_default[] = "F:\\GreatHomework";
...
int judge(char b[MAXLINE]){                                                    
    ...
    if(b[0]=='d'&&b[1]=='i'&&b[2]=='f'&&b[3]=='f'&&b[4]=='\0')        //检测下一位是否为\0 防止误判 正确指令+多余字符 如pwdf 
        return 1;
    if(b[0]=='g'&&b[1]=='r'&&b[2]=='e'&&b[3]=='p'&&b[4]=='\0')
        return 2;
    if(b[0]=='t'&&b[1]=='e'&&b[2]=='e'&&b[3]=='\0')
        return 3;
    if(b[0]=='c'&&b[1]=='a'&&b[2]=='t'&&b[3]=='\0')
        return 4;
    if(b[0]=='c'&&b[1]=='p'&&b[2]=='\0')
        return 5;
    if(b[0]=='c'&&b[1]=='d'&&b[2]=='\0')
        return 6;
    if(b[0]=='p'&&b[1]=='w'&&b[2]=='d'&&b[3]=='\0')
        return 7;
    if(b[0]=='e'&&b[1]=='c'&&b[2]=='h'&&b[3]=='o'&&b[4]=='\0')
        return 8;
    if(b[0]=='e'&&b[1]=='x'&&b[2]=='i'&&b[3]=='t'&&b[4]=='\0')
        return 9;
    cerr<<"command '"<<b<<"' not found";                      //未检测到命令，直接将错误提示打在屏幕上 
        return 0;
}
